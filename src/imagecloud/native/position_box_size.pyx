...
cdef empty_position():
    return to_position(-1,-1)

cdef is_empty_position(Position pos):
    return pos.left == -1 and pos.upper == -1
...
cdef is_empty_box(BoxCoordinates box):
    return box.left == -1 and box.upper == -1 and box.right == -1 and box.lower == -1
...
def py_sample_resize_to_area(Size size, int area, int step_size, int maintain_aspect_ratio):
